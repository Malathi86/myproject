	...
	var DataTable = function( oInit )
	{
		...
		function _fnUpdateInfo ( oSettings )
		{
			...
			var
				...
				iMax = oSettings.fnRecordsTotal(), iTotal = "", //$("#noplayers").text(), //iTotal = oSettings.fnRecordsDisplay(),
				sStart = oSettings.fnFormatNumber( iStart ), sEnd = oSettings.fnFormatNumber( iEnd ),
				sMax = oSettings.fnFormatNumber( iMax ), sTotal = "", //sTotal = oSettings.fnFormatNumber( iTotal ),
				sOut;
			/* Added by Malathi */
			if(window.location.pathname == "/messages" || window.location.pathname == "/login" || window.location.pathname == "/home"){				
				iTotal = $("#noplayers").text();
		...
		function _fnSort ( oSettings, bApplyClasses )
		{
			...
			if(_smSort == 0){			
				...
				if(_smSort == 2) {
					...
					for ( i=0 ; i<aaSort.length ; i++ )
					{						
						var iVisColumn = _fnColumnIndexToVisible( oSettings, aaSort[i][0] );	
						sortTable(iVisColumn, aaSort[i][1]);
					}
				}
			}
		...
		this.fnAddData = function( mData, bRedraw )
		{
			...
			var oSettings = _fnSettingsFromNode( this[DataTable.ext.iApiIndex] );
			var curr_start = oSettings._iDisplayStart;
			...
			{						
				oSettings._iDisplayStart = Math.ceil(curr_start / oSettings._iDisplayLength) + 1;
				_fnReDraw( oSettings );				
			}
			return aiReturn;
		};
		...
		"full_numbers_pp": {
			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
					...
					var fnClickHandler = function ( e ) {
						...
						{
							fnCallbackDraw( oSettings );
							if($("#jqtable_next").hasClass("ui-state-disabled")) $("#jqtable_next").removeClass("ui-state-disabled");
						}
					};
					/* Code added by Me */
					var fnClickHandlerForNext = function ( e ) {							
						if ( oSettings.oApi._fnPageChange( oSettings, e.data.action ) )
						{								
							loadNextSet();	
							fnCallbackDraw( oSettings );
					...
					oSettings.oApi._fnBindAction( nNext,  {action: "next"},     fnClickHandlerForNext );				
			...
			"fnUpdate": function ( oSettings, fnCallbackDraw )
			{
				...
				var iPages = Math.ceil($("#noplayers").text() / oSettings._iDisplayLength);
				...
				for ( i=0, iLen=an.length ; i<iLen ; i++ )
				{
					...
					anStatic = [
						anButtons[0], anButtons[1], anButtons[anButtons.length-1]
